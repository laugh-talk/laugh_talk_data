...
parser.add_argument('--outdir', type=str, default="./processed_talk", help='Path outputs')
...
def main():
    ...
    # ```
    cnt = 0
    args.outdir = os.path.join(args.outdir,"processed_talk")
    os.makedirs(args.outdir, exist_ok=True) 
    for vid_ in os.listdir(args.videoFolder):
        
        args.videoPath = glob.glob(os.path.join(args.videoFolder, vid_))[0]
        args.savePath = os.path.join("./delete", vid_.split(".")[0])
        ...
        if len(scores) == 0:
            continue
        # if negative score len is longer, delete
        if len(scores[0][scores[0]<0])<len(scores[0][scores[0]>0]):
            outpath = os.path.join(args.outdir, vid_)
            shutil.copy(args.videoPath, outpath)
